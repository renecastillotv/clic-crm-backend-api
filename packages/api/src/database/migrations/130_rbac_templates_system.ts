...
export async function up(knex: Knex): Promise<void> {
  ...
  const hasEsSubmenu = await knex.schema.hasColumn('modulos', 'es_submenu');
  if (!hasEsSubmenu) {
  ...
  const hasPermisosCampos = await knex.schema.hasColumn('roles_modulos', 'permisos_campos');
  if (!hasPermisosCampos) {
  ...
  const hasPermisosVersion = await knex.schema.hasTable('permisos_version');
  if (!hasPermisosVersion) {
