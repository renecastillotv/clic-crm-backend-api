...
async function run() {
  ...
  try {
    ...
    if (checkCol.rows.length > 0) {
      console.log('⚠️ Column asesor_default_id already exists, skipping creation');
    } else {
      // Add column
      await client.query(`
        ALTER TABLE tenants
    ...
    await client.query(`
      ...
      VALUES ('129_add_asesor_default_to_tenants', NOW())
      ON CONFLICT (name) DO NOTHING
    `);

    await client.query('COMMIT');
    console.log('✅ Migration 129 completed successfully');
